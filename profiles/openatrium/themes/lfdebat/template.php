 ...
 */
function lfdebat_icon_links(&$links) {
  if (!empty($links)) {
    foreach ($links as $k => $v) {
      ...
      $v['attributes']['class'] .= ' icon-'. _lfdebat_icon_class($v['href']);
 ...
 */
function lfdebat_preprocess_page(&$vars) {
  ...
  if (arg(0) === 'admin') {
    ...
    lfdebat_icon_links($vars['primary_links']);
  }
  ...
  $ie = base_path() . drupal_get_path('theme', 'lfdebat') .'/ie.css';
 ...
 */
function lfdebat_preprocess_block(&$vars) {
 ...
 */
function lfdebat_preprocess_context_block_editable_region(&$vars) {
 ...
 */
function lfdebat_preprocess_help(&$vars) {
  $vars['layout'] = FALSE;
  $vars['links'] = '';
}
 ...
 */
function lfdebat_preprocess_node(&$vars) {
  if (!empty($vars['terms'])) {
    $label = t('Tagged');
 ...
 */
function lfdebat_preprocess_comment(&$vars) {
  // Add a time decay class.
  $decay = _lfdebat_get_comment_decay($vars['node']->nid, $vars['comment']->timestamp);
 ...
 */
function lfdebat_preprocess_node_form(&$vars) {
 ...
 */
function lfdebat_designkit_image($name, $filepath) {
  if ($name === 'logo') {
 ...
 */
function lfdebat_more_link($url, $title) {
 ...
 */
function lfdebat_breadcrumb($breadcrumb) {
 ...
 */
function lfdebat_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
 ...
 */
function lfdebat_views_mini_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
  $tags[1] = t('Prev');
  $tags[3] = t('Next');
 ...
 */
function lfdebat_node_preview($node = NULL, $show = FALSE) {
  static $output;
 ...
 */
function lfdebat_content_multiple_values($element) {
 ...
 */
function lfdebat_node_submitted($node) {
 ...
 */
function lfdebat_comment_submitted($comment) {
  $comment->created = $comment->timestamp;
  return lfdebat_node_submitted($comment);
}
 ...
 */
function lfdebat_preprocess_views_view_fields(&$vars) {
  foreach ($vars['fields'] as $field) {
    if ($class = _lfdebat_get_views_field_class($field->handler)) {
      $field->class = $class;
    }
  }
 ...
 */
function lfdebat_preprocess_views_view_table(&$vars) {
  $view = $vars['view'];
  foreach ($view->field as $field => $handler) {
    if (isset($vars['fields'][$field]) && $class = _lfdebat_get_views_field_class($handler)) {
 ...
 */
function _lfdebat_get_views_field_class($handler) {
 ...
 */
function _lfdebat_get_comment_decay($nid, $timestamp) {
  static $timerange;
 ...
 */
function _lfdebat_icon_class($path) {
